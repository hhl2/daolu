<!--检测-->
<template>
  <div class="process_box" v-if="showParent">
    <div class="process_box_top">
      ...
      <div class="process_position">
        <div class="tc_box">
          <div class="tc_left">检测列表-{{projectInfo.projectName}}</div>
          <div class="tc_right">
            <el-icon @click="goToHome">
              <CloseBold/>
            </el-icon>
        ...
        <div class="roadlist_table">
          ...
          <el-table
            ...
            <el-table-column prop="sysAreaRoad.roadCsdlLevel_dictText" label="道路等级"/>
            <el-table-column prop="sysAreaRoad.checkNum" label="检测情况"/>
  ...
  <router-view v-else name="detection"/>

</template>
<style></style>
<script>
export default {
  name: 'ProcessDetection' // 改为多单词名称
}
</script>
...
import { get_lc, get_disease_type, get_xiangmu, get_xiangmu_daolu} from '@/api/user'
...
onMounted(async () => {
  ...
  await get_xiangmu({}).then(res=>{
    ...
    if(res.result.records){
      ...
      //病害类型及数量统计
      get_disease_type({projectId: project.id}).then(res => {
