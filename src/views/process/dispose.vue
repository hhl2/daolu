  ...
  <div class="process_box">
    <div class="process_box_top">
      <div class="process_chart_position" style="margin-right:20px;">
        <div class="tc_box">
          <div class="tc_left">本期病害处置情况</div>
          <div class="tc_right">
      ...
      <div class="process_position">
        ...
        <div class="roadlist_table">
          ...
          <el-table
            ...
            <el-table-column prop="status_txt" label="待处置">
              <template #default="{ row }">
                        <span v-if="repeatData[row.roadName] !== undefined">
                        {{ repeatData[row.roadName].waitFix }}
                        </span>
              </template>
            </el-table-column>
            <el-table-column prop="follow_up_user_name" label="不需处置">
              <template #default="{ row }">
                        <span v-if="repeatData[row.roadName] !== undefined">
                        {{ repeatData[row.roadName].sst }}
                        </span>
              </template>
            </el-table-column>
            <el-table-column prop="follow_up_user_name" label="已处置">
              <template #default="{ row }">
                        <span v-if="repeatData[row.roadName] !== undefined">
                        {{ repeatData[row.roadName].fixed }}
                        </span>
              </template>
            </el-table-column>
          </el-table>
            ...
            series: [
              {
                ...
                label: {
                  normal: {
                    ...
                    formatter: '{name|' + (parseInt(jc_result.value.fixed, 10) + parseInt(jc_result.value.waitFix, 10) + parseInt(jc_result.value.sst, 10)) + '}\n{time|总计}',
                ...
                data: [{"value": jc_result.value.fixed, "name": "已处置"}, {
                  "value": jc_result.value.waitFix,
                  "name": "待处置"
                }, {"value": jc_result.value.sst, "name": "不需处置"}]
              },
              {
