  ...
  <div class="roadlist_box yy_box">
    ...
    <div class="info_box yy_table">
      <el-descriptions
          ...
          :style="blockMargin"
      >
        <el-descriptions-item label="病害编号">{{ yyxc || '' }}</el-descriptions-item>
        <el-descriptions-item label="病害影响范围">12平米</el-descriptions-item>
        <el-descriptions-item label="影响管线类型">供水</el-descriptions-item>
        <el-descriptions-item label="影响管线数量">1</el-descriptions-item>
        <el-descriptions-item label="可能成因">管线漏水</el-descriptions-item>
        <el-descriptions-item label="阀门">关闭2个</el-descriptions-item>
        <el-descriptions-item label="病害承载力">11吨</el-descriptions-item>
...
const fetchData = async (disastercode) => {
  try {
    ...
    get_disease({
      ...
      searchtext: disastercode,
      riskLevel: riskLevel.value,
      disasterType: disasterType.value,
      status_dictText: status_dictText.value,
      roadName: searchroad.value
    }).then(res => {
      console.log('预演弹窗-ue:', {current: currentPage.value, pagesize: pageSize.value, searchtext: disastercode})
...
onBeforeMount(() => {
  //1.获取本地存储值
  var roaddata = localStorage.getItem('yyxc');
  // 2. 解析 JSON 数据（带错误处理）
  try {
    const roadinfo = roaddata ? JSON.parse(roaddata) : null;
    console.log('预演信息:', roadinfo.id);
    yyxc.value = roadinfo.id;
  } catch (e) {
