<template>
  <div class="roadlist_box" v-if="showParent">
      ...
      <div class="search-box">
        ...
        <div class="road_tab binghai_tab">
          <el-tabs v-model="activeName" class="demo-tabs">
            ...
            <!-- <el-tab-pane label="空间分析" name="four">
            </el-tab-pane> -->
        ...
        <el-table-column prop="roadName" label="道路名称" show-overflow-tooltip="true">
          <template #default="scope">
            ...
            <a
                ...
                @click="disastercodeclick(scope.row.roadName,'daolu'),handleClick(scope.row.roadName)"
            >
              {{ scope.row.roadName }}
            </a>
          </template>
    ...
    <el-dialog @closed="handleDialogClose" v-model="dialogVisible" width="50%" style="text-align:center;">
      <router-view name="road"></router-view>
    </el-dialog>
  </div>

  <router-view v-else name="road"/>
</template>
<style>
...
watch(
    () => route.name,
    (newVal) => {
      dialogVisible.value = newVal === 'road_modal' // 根据子路由激活状态控制弹窗
    },
    {immediate: true}
)
// 弹窗关闭时清理路由
const handleDialogClose = () => {
  router.push({name: 'road_list'}) // 返回父路由
}
const handleClick = (item) => {
  localStorage.setItem('roadinfo', JSON.stringify({id: item}))
  router.push({
    name: 'road_modal', // 使用子路由名称
    query: {id: item} // 可选参数
  })
}
