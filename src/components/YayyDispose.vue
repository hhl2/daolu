...
<template>
  <div class="roadlist_table">
    ...
    >
      ...
      <el-table-column prop="fixType_dictText" label="推荐处置方法">
        <template #default="{ row }">
          {{ row.fixType_dictText?row.fixType_dictText:'加密观测' 
          }}
        </template>
      </el-table-column>
      <el-table-column prop="follow_up_user_name" label="推荐处置用料(m³)">
        <template #default="{ row }">
          {{
      ...
      <!-- <el-table-column prop="follow_up_user_name" label="面积">
        <template #default="{ row }">
          {{
             row.square
          }}
      ...
      <el-table-column prop="follow_up_user_name" label="净深">
       <template #default="{ row }">
          {{
             row.depth
          }}
  ...
  <el-dialog @closed="handleDialogClose" v-model="dialogVisible" width="50%" style="text-align:center;">
    <router-view name="cz"></router-view>
  </el-dialog>
</template>

<script setup>
import {inject, onMounted, reactive, ref, watch} from 'vue'
import {get_disease, get_work} from '@/api/user'
...
watch(
    () => route.name,
    (newVal) => {
      dialogVisible.value = newVal === 'cz_modal' // 根据子路由激活状态控制弹窗
    },
    {immediate: true}
)
// 弹窗关闭时清理路由
...
//用料算法
const calculateMaterial = (row) => {
  var square = 0;
  var depth = 0;
  if (row.disasterType_dictText.includes('空洞') || row.disasterType_dictText.includes('脱空') ) {
     
      square =  row.square || 0
      depth =  row.depth || 0 
    return 1.1 * square * depth > 0 ? (1.1 * square * depth ).toFixed(2) : '无需用料';
  } else if (['疏松体', '富水体'].some(type =>
      row.disasterType_dictText.includes(type))) {
    return '无需用料';
  }
  return '';
}
...
const handleFilterChange = (filters) => {
  ...
  if (filters.status_dictText) {
    if (filters.status_dictText[0] == null) {
      status_dictText.value = '';
    } else {
      status_dictText.value = filters.status_dictText[0];
    }
    console.log('触发事件病害状态', status_dictText.value);
  }
...
const repeat_dis = async (roadname) => {
  if (repeatData[roadname]) return

  try {
    //获取路网列表
    get_work({current: 1, pagesize: 1, searchtext: roadname}).then(res => {
      console.log('道路信息:', res.result.records[0])
      repeatData[roadname] = res.result.records[0]
    }).catch(err => {
      console.error('道路信息请求失败1:', err)
    });
