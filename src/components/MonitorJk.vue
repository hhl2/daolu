...
let trackPolyline = null;  // 轨迹折线对象
 const initMap = () => {
      ...
      if (mapContainer && !map) {
        ...
        const point = new window.BMapGL.Point(104.06, 30.70);
...
// 创建当前坐标点
                  let newPoint = new window.BMapGL.Point(longitude, latitude);
                  ...
                  if(marker){
                      ...
                      marker = new window.BMapGL.Marker(newPoint, {
                          icon: new window.BMapGL.Icon(
                              './marker.png',
                              new window.BMapGL.Size(48, 48), {
                                  offset: new window.BMapGL.Size(24, 48),
                              }
                          )
                      });
                  ...
                  // 2. 如果折线不存在则创建
                  if (!trackPolyline) {
                      trackPolyline = new window.BMapGL.Polyline(trackPoints, {
                          strokeColor: "#FF3300",
                          strokeWeight: 4,
                          strokeOpacity: 0.7,
                          lineStrokeType: "arrow"
                      });
                      map.addOverlay(trackPolyline);
                  }
                  // 3. 如果折线已存在则更新路径
                  else {
                      trackPolyline.setPath(trackPoints);
                  }
                  // ========== 轨迹绘制结束 ==========
